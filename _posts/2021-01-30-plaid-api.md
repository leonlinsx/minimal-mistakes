...
And if you can make that library accessible to everyone publicly, then a lot of people could use it to build even more interesting things. You can focus on making your math functions, and others can focus on making matrix multiplication apps by using your library as a building block.
...
We'll call that library of functions an API. Joshua believes that the first use of the term comes from [a 1968 paper by Ira Cotton and Frank Greatorex:](https://www.computer.org/csdl/pds/api/csdl/proceedings/download-article/12OmNyRPgFZ/pdf "ira")
...
- Hardware independence. By having a layer of abstraction, we want to be able to use the API's functions regardless of what device we're using
- Reusability. The library can be used across multiple people who want different things

Using our API requires well defined contracts telling us the inputs to our functions and the outputs expected. For example, we want the sum() function to return the total of all input numbers wherever it's used, and not the total in some cases and the average in others.
...
Imagine life without any APIs. That would mean you had to start from scratch every time you programmed anything. We all work on one layer of a problem, and rely on the lower levels being solved for us already.
...
Plaid is essentially April in our earlier example. 
